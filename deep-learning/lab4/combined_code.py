...
class RNN:
    ...
    def ada_grad(self):
        ...
        for _ in range(self.num_epochs):
            ...
            for j in range(0, self.num_chars - self.seq_length, self.seq_length):
                ...
                d_L_d_V, d_L_d_W, d_L_d_U, d_L_d_b, d_L_d_c = self.compute_gradients(X=X, Y=Y, a=a, h=h, p=p)
                ...
                loss = self.compute_loss(X=X, Y=Y, h_prev = h_prev)
    ...
    def compute_gradients(self, X, Y, a, h, p):
        num_cols = X.shape[1]
    ...
    def compute_loss(self, X, Y, h_prev):
        return -np.sum(Y * np.log(self.forward_pass(X=X, h_prev=h_prev)[2]))
    ...
    def forward_pass(self, X, h_prev):
        ...
        for i in range(num_cols):
            a_t = self.W @ h_t + self.U @ X[:, i].reshape(-1, 1)
...
def check_gradients(num_sequences = 30):
    ...
    for i in range(num_sequences):
        ...
        L_sum, d_L_d_V, d_L_d_W, d_L_d_U, d_L_d_b, d_L_d_c = rnn_an.compute_gradients(X_arr=X, Y_arr=Y, h_prev=h_prev)
