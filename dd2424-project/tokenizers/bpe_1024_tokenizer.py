...
def process_file(file_path, output_prefix, desired_vocab_size):
    ...
    np.save(f"{output_prefix}_bpe_1024.npy", bytes_list)

    with open(f'{output_prefix}_vocabulary_bpe_1024.pkl', 'wb') as f:
        pickle.dump(vocab, f)
...
if __name__ == "__main__":
    ...
    train_merges = process_file("data/train.txt", "token_data/train", desired_vocab_size=1024)
    print("Vocabulary created")
    print("Encoding validation.txt")
    with open("data/validation.txt", "r") as f:
       val_string = f.read()
    val_encoded = encode(val_string, train_merges)
    np.save(f"token_data/validation_bpe_1024.npy", val_encoded)

    print("Encoding test.txt")
    with open("data/test.txt", "r") as f:
       test_string = f.read()
    test_encoded = encode(test_string, train_merges)
    np.save(f"token_data/test_bpe_1024.npy", test_encoded)
